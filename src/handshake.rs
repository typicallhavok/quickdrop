...
use crate::protocol::{
    ACCEPT, HEADER_SIZE, IDENTITY_CHALLENGE, IDENTITY_HELLO, IDENTITY_PROOF, OFFER_ACCEPT, OFFER_REJECT, REJECT, make_header, parse_header
};
...
impl<'a> HandshakeContext<'a> {
    fn new(store: &'a mut Store, store_path: &'a str, local_public_key: [u8; 32]) -> Self {
        Self {
            store,
            store_path,
            state: State::New,
...
pub fn run_handshake<'a>(
    ...
    let mut ctx = HandshakeContext::new(store, store_path, *local_public_key);
    ...
    let session_key = derive_session_key(server_nonce, client_nonce, &ctx.local_public_key, peer_key);
...
fn handle_identity_proof(
    ...
    if !verify_signature(peer_public_key, server_nonce, client_nonce, server_public_key, signature) {
        send_handshake_reject(stream)?;
    ...
    let trusted = is_trusted(ctx.store, peer_public_key);

    send_handshake_accept(stream, trusted)?;
    Ok(())
}

fn verify_signature(public_key: &[u8], nonce: &[u8], client_nonce:&[u8], server_public_key:&[u8], signature: &[u8]) -> bool {
...
fn send_handshake_accept(stream: &mut TcpStream, auto_send: bool) -> io::Result<()> {
    let payload = [auto_send as u8];

    write_message(stream, ACCEPT, &payload)
}

fn send_handshake_reject(stream: &mut TcpStream) -> io::Result<()> {
